...
describe('/reset-password GET', () => {
    describe('correct request', () => {
        let response;
        beforeAll(async () => {
            response = resetPasswordEmailRequest({});
        })
    ...
    describe('wrong request', () => {
        describe('request with wrong access token', () => {
            ...
            beforeAll(async () => {
                ...
                response = await resetPasswordEmailRequest({
    ...
    const resetPasswordEmailRequest = (options) => {
...
describe('/reset-password/:token POST', () => {
    describe('correct request', () => {
        ...
        beforeAll(async () => {
            ...
            response = await resetPasswordRequest(
                {
                    data: { newPassword: 'newPerfectPassword', password: validUser.password },
                    oneTimeToken: oneTimeToken
                });
        })

        afterAll(async () => {
            await User.updateOne({ username: validUser.username }, validUser)
        })
        ...
        it('password should be changed', async () => {
            ...
            expect(user.password).toBe('newPerfectPassword');
        })
    })
    ...
    const resetPasswordRequest = (options) => {
        const { data, oneTimeToken, customToken } = options;
        return makeHttpRequest(app, {
            method: 'POST',
            endpoint: `/reset-password/${oneTimeToken}`,
            isIncludeToken: true,
            customToken: customToken,
            data: data
        });
